...
| `power_automate_report.md` | **Optional.** When the **Power Automate Report** action is connected: **`submitEvaluationReport`** JSON (`WorkbookSheetPayload`), one POST per Excel sheet (`sheetName`), field mapping from Markdown to JSON schema, payload size guidance, flow design reference |
