...
**System Context:** When the ERD includes a System Context block, use **`system_context_schema.md`** only for **§4a / §4b triage**, inline mentions in Completeness or Recommendations where a **specific** gap references context, and **hallucination counting rules**—**not** for a dedicated narrative section in Step 1 (no external GitHub/Confluence verification).
...
### System Context (Step 1) — no dedicated section

If the generated ERD includes a **System Context** block:

- **Do not** add any System Context line to the §2 **Quantitative Metrics** summary table.
- **Do not** add a subsection titled **System Context (when present)** or any standalone block that summarizes structural completeness, context–PRD alignment, context relevance, or unsupported/contradictory context claims (no dimension **%** prose for System Context in Step 1).
- **Do** still use in-document System Context text for **§4a / §4b**, the **Hallucination rate** definition above, **Completeness**, **Traceability**, and **Recommendations** where a specific item warrants a short pointer to context (inline only).

**Do not** verify against live GitHub or Confluence.
