...
1. **Project name** (for the visible report title): Prefer the **PRD product or program name**—from PRD title page, document properties, or the main H1. If missing, derive from the **PRD filename** (strip `.docx`, trim “PRD”, “v2”, dates). If still unclear, ask once: *What project name should appear on the Excel report?*
...
## User-facing names (never “Step 1/2/3” on sheets)
...
## Workbook size: at most 4 sheets
...
Alternative if you prefer numeric prefixes: `01_Overview`, `02_PRD_Analysis`, `03_Eng_ERD_Comparison`, `04_Scoped_Alignment`.
...
| Evaluations included | e.g. “PRD Analysis; Engineering ERD Comparison; Scoped alignment review” |
...
2. **Quantitative metrics** — full table (metric, result, basis, confidence, why) if present in report.
...
3. **Structure check ledgers** — all sections (long format).
...
1. **All completed evaluations** appear: Overview scores + every detail sheet for phases that ran.
